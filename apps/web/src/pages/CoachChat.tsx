...
const CONVO_KEY = 'pulse_coach_convo';
...
export default function CoachChat() {
  ...
  return (
    <div className="flex min-h-screen flex-col pb-0">
      <TopBar title={t('coachChat.title')} color="fitness-hero" textColor="text-white" />
        ...
        {messages.length === 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-white p-4 shadow-sm">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br from-orange-500 to-pink-600 text-white">
                <Bot size={20} />
              </span>
              <div>
                <p className="font-bold">{t('coachChat.hello')}</p>
                <p className="text-xs text-gray-500">{t('coachChat.sub')}</p>
              </div>
            </div>
          ...
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
                m.role === 'user' ? 'rounded-br-md bg-gradient-to-r from-orange-500 to-pink-600 text-white' : 'rounded-bl-md bg-white shadow-sm'
              }`}
            >
              {m.text}
            </div>
      ...
      <div className="fixed inset-x-0 bottom-0 z-20 mx-auto max-w-[480px] border-t border-white/10 bg-ink/90 p-3 backdrop-blur" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 12px)' }}>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-gradient-to-r from-orange-500 to-pink-600 text-white disabled:opacity-50"
          >
