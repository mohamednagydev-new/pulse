              ...
              {detail.participants.map((p: any, i: number) => (
                <div key={p.userId} className={`rounded-xl border p-3 ${p.suspicious ? 'border-amber-300 bg-amber-50' : 'border-gray-100'}`}>
                  ...
                  <p className="mt-1 text-[11px] text-gray-500">
                    {p.email} · account {p.accountAgeDays}d old · {p.totalXpInWindow} XP in window · max/day: {p.maxXpDay} XP, {p.maxWorkoutsDay} workouts, {p.maxCalDay} kcal · {p.entryCount} food entries
                  </p>
                  {p.flags.length > 0 && (
