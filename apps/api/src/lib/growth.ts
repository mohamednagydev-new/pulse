...
- SPONSOR (راعي): not an ad, a branded ritual — prize challenge with their name on the challenge card AND a «برعاية» banner inside the challenge room that every participant sees daily for the whole run, plus a Home banner during the prize window, podium + raffle + permanent Wall of Champions, winners-announcement post with their brand, events board with RSVP attribution. Founding sponsors pay nothing — their only cost is the prize itself (product bundles/vouchers); only ONE sponsored challenge runs at a time, which is why the slot is worth taking now.
