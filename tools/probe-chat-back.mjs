...
const B = 'https://pulse.geddo.online';
...
page.on('console', (m) => m.type() === 'error' && console.log('CONSOLE ERR:', m.text().slice(0, 180)));
...
await page.locator('input[autocomplete="email"], input[type="email"]').first().fill('probe_chat_a_1786454646@test.local');
...
await page.goto(`${B}/chat/cmsop0tuo00e2ua5oodi567ab`, { waitUntil: 'domcontentloaded' });
await page.waitForTimeout(2500);
